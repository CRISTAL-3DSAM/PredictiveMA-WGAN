...
LABELS={'Directions','Discussion','Eating','Greeting','Phoning','Photo','Posing','Purchases','Sitting','SittingDown','Smoking','Waiting','WalkDog',...
    'Walking','WalkTogether'};
if ~any(strcmp(LABELS,class))
    fprintf('Wrong class name please use one of the correct class names\n\n');
    fprintf('''Directions'',''Discussion'',''Eating'',''Greeting'',''Phoning'',''Photo'',''Posing'',''Purchases'',''Sitting'',''SittingDown'',''Smoking'',''Waiting'',''WalkDog'',''Walking'',''WalkTogether''\n')
    return
end
...
for i=1:N_samples
    ...
    skeleton_path = ['Data_skeleton\Skeletons_for_visu_' suffix '\Subject_5\', class,...
        '\' num{1}, '\skeleton.mat'];   
    norm_path = ['Data_skeleton\SRVF_Intensity_' suffix '\Subject_5\', class,...
        '\' num{1}, '_SRVF_intensity.mat'];
    
    path_mean = ['Data_skeleton\MEANS_' suffix '\Subject_5\', class,...
        '\' num{1}];
    path_fronorm = ['Data_skeleton\NORMS_' suffix '\Subject_5\', class,...
        '\' num{1}];
    path_center = ['Data_skeleton\CENTERS_' suffix '\Subject_5\', class,...
        '\' num{1}];
    ...
    for t=36:50
        ...
        drawnow;
        hold(ax1,'off');
        axis(ax1,[-1000 1000 -1000 1000 -1000 1000]);
        view(ax1,-45,0)
        cla(ax2);
        hold (ax2,'on')
        ...
        drawnow;
        hold(ax2,'off');
        axis(ax2,[-1000 1000 -1000 1000 -1000 1000]);
        view(ax2,-45,0)
